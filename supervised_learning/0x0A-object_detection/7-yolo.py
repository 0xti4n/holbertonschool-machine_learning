...
class Yolo():
    ...
    def predict(self, folder_path):
        """Predict images"""
        predictions = []
        data_p, img = self.load_images(folder_path)
        pimgs, img_shapes = self.preprocess_images(data_p)
        outputs = self.model.predict(pimgs)
        for i in range(len(outputs[0])):
            list_boxes = []
            for j in range(len(outputs)):
                list_boxes.append(outputs[j][i])
            boxes, b_conf, b_cls_prob = self.process_outputs(list_boxes,
                                                             img_shapes[i])
            boxes, b_cls, b_score = self.filter_boxes(boxes, b_conf,
                                                      b_cls_prob)
            boxes, b_cls, b_score = self.non_max_suppression(boxes,
                                                             b_cls, b_score)
            predictions.append((boxes, b_cls, b_score))
            self.show_boxes(data_p[i], boxes, b_cls, b_score,
                            img[i].split('/')[-1])
